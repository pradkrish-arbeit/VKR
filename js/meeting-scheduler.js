document.getElementById('meeting-scheduler-form').addEventListener('submit', function (event) {
  ...
  async function generateSchedule() {
    try {
      ...
      const response = await fetch("https://my-json-server.typicode.com/pradkrish-arbeit/VKR/users");
      ...
      fetch('https://my-json-server.typicode.com/pradkrish-arbeit/VKR/meetings_calender')
        .then(response => response.json())
        .then(schedules => {
          // Iterate over each employee and delete them
          const deletePromises = schedules.map(schedule => {
            return fetch(`https://my-json-server.typicode.com/pradkrish-arbeit/VKR/meetings_calender/${schedule.id}`, {
              method: 'DELETE'
            });
          });
